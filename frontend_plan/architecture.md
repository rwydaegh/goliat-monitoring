...
- **Database**: Vercel Postgres (Neon) - Free tier: ~3M row reads/month
- **File Storage**: Vercel Blob - Free tier: 1GB storage, 10GB/month transfer
...
### 1. Storage Strategy
- **Vercel Postgres** for all state/metadata (workers, assignments, progress events)
- **Vercel Blob** for artifact storage (configs, results, logs)
- **Rationale**: Keeps everything within Vercel ecosystem, free tier sufficient for ~100MB total artifacts
...
3. Server stores split configs in Blob, creates assignments in Postgres
...
6. Server stores in Postgres `gui_state` table
7. Dashboard polls `/api/workers` every 2-3 seconds, displays updates

### Completion
1. Study finishes, sends `{"type": "finished"}` message
2. Worker agent uploads artifacts to Blob (optional)
3. Worker agent marks assignment complete in Postgres
4. Dashboard shows worker as "idle" again

## Scalability Considerations

- **8-16 workers**: Well within Vercel function limits (30k concurrent executions)
- **Update frequency**: Throttled to 5Hz per worker = ~40-80 req/s total (manageable)
- **Storage**: 100MB total artifacts fits comfortably in free tier
- **Postgres**: 3M reads/month ≈ 100k/day ≈ 70/minute = enough for dashboard polling
...
- **Rate limiting**: Consider Vercel middleware for API protection
